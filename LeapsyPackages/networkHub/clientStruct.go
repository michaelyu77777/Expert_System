			...
			for { // 循環讀取連線傳來的資料
				...
				if ws.OpText == wsOpCode {
					...
					case 1: // 登入(+廣播改變狀態)
						...
						// demo 帳號完全可以登入
						check, accountPointer = cTool.checkPassword(command.UserID, command.UserPassword)
					...
					case 15: // 判斷帳號是否存在，若存在則寄出驗證信
						...
						// 是否有此帳號
						haveAccount, accountPointer := cTool.checkAccountExist(command.UserID)
					...
					case 17: // QRcode登入 (+廣播)
						...
						// 是否有此帳號
						check, accountPointer := cTool.checkAccountExist(userid)
					...
					case 18: // 眼鏡切換場域
						...
						// 檢查欄位是否齊全
						if !cTool.checkFieldsCompletedAndResponseIfFail([]string{"area"}, clientPointer, command, whatKindCommandString) {
							break // 跳出case
						}
